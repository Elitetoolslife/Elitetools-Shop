...
function displayWithdrawalRequests($dbcon) {
            ...
            <table class="table table-bordered table-striped">
              ...
              while($row = mysqli_fetch_assoc($q)){
                  ...
                  echo '<tr>
                      ...
                      <td>'.$receivejer.'</td>
                      <form>
                      <td><a target="_blank" href="PaySeller.html?seller='.$row['username'].'&receiveusd='.$receive.'&receivebtc='.substr($convertedCost, 0, 9).'&btcaddress='.$row['btc'].'&pending='.$pending_orders.'">[...]
                      </form>
                      </td>
                      </tr>';
              }
              echo '<tr><td>TOTAL</td> <td></td><td></td><td></td><td>'.$totalseller.'</td><td></td><td></td><td>'.$totaljerux.'</td><td></td></tr>';
              echo '
              </tbody>
            </table>
        </div>
...
You can call the `displayWithdrawalRequests` function and pass the database connection `$dbcon` to it.

```php
displayWithdrawalRequests($dbcon);
```